...
/**
 * A list fragment representing a list of edu.pdx.ece558.learnbraille.Lessons. This fragment
...
public class LessonListFragment extends ListFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 0; i < mLessonDescriptions.length; ++i){
            if(mLessonTitles[i].isEmpty()  || mLessonDescriptions[i].isEmpty()){
                Log.v(TAG, "Cannot find title/description for " + i);
            }else{
                mLessons.add(new Lesson(i,mLessonTitles[i],mLessonDescriptions[i]));
            }
        }

        setListAdapter(new ArrayAdapter<Lesson>(
                getActivity(),
    ...
    public void onListItemClick(ListView listView, View view, int position, long id) {
        ...
        mCallbacks.onItemSelected(mLessons.get(position).id);
    }
